...
## Model ##
###########
for par_set = {"Ss"} ## use this for starters, check with Andrea & Martin whether additional soil textures should be included
  ...
  ## for bound_con = [5]
	...
	filename_prefix = [par_set, "_", scenario, "_", num2str(bound_con), "_", num2str(init_con), "_"];
	...
	system(["rm -fv ", attempt_file]);
	## Begin “catch” block:
        catch
	[msg, msgid] = lasterr();
	## sprintf("last error: %s", msg)
	## Create dummy file to mark modelling attempt for the given parameter setting, scenario and conditions:
	textbox = {sprintf("%s%s\n", "par_set = ", par_set),
		 sprintf("%s%s\n", "H_top = ", num2str(H_top)),
		 sprintf("%s%s\n", "H_bot = ", num2str(H_bot)),
