...
The relationships between $\theta$ and $h$ on the one hand, and $K$ and $h$ on the other hand are nonlinear \parencite{celia_general_1990}.  Therefore, numerical approximation is a widely used approach for solving equation (\ref{eq:richeq_pde}), requiring both temporal discretization (i.e., discretization with respect to $t$) and spatial discretization (i.e., discretization with respect to $x, y, \text{and } z$) of the equation.
